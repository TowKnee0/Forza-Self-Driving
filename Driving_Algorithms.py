...
def drive_max_dist(lanes, screen_size, pressed):
    """This algorithm is an attempt to drive the car based on predetermined instructions.
...
def drive_max_dist(lanes, screen_size, pressed):
    ...
    if len(lanes) == 2 and any({slope > 0 for slope in slopes}) \
        ...
        if not pressed:
            ...
            if right_dist - left_dist < thresh  or left_dist - right_dist < thresh :
                key_press.ReleaseKey(0x20)
                key_press.ReleaseKey(0x1E)
                press_status = False
    ...
    # elif len(lanes) == 1:
    #     if slopes[0] < 0 and point_to_line_dist(car, lanes[0]) < 80:
    #         # key_press.PressKey(0x1E)
    #         # press_status = True
    #         key_press.right()
    #     elif slopes[0] > 0 and point_to_line_dist(car, lanes[0]) < 80:
    #         # key_press.PressKey(0x20)
    #         # press_status = True
    #         key_press.left()
